...
Los siguientes dos puntos deben estar en un repositorio con el nombre `NombreApellido_Ejercicio25`

 a) Escriba un programa en C o C++ (`sample.c`) que al ejecutarse como `./sample N mean sigma`, donde `N` es un entero, genere `N` puntos de 
...
b) Escriba un programa en pytho que lea los resultados de `sample.dat`, haga un histograma normalizado de los datos y lo compare con
